...
#         # Create an Image instance and associate it with the product
#         image_instance = Image(product=product)
        
#         img_io = BytesIO()
#         img.save(img_io, format='PNG')  # You can adjust the format based on your needs
#         image_instance.image.save(f'{product.id}_image.png', img_io, save=True)
...
def download_and_save_image(product, image_url):
    ...
    if response.status_code == 200:
        ...
        img_io = BytesIO()
        img.save(img_io, format='PNG')  # Adjust format as needed
        img_io.seek(0)

        # Save the file to CloudinaryField
        image_instance.image.save(f'{product.id}_image.png', ContentFile(img_io.read()), save=True)
...
def import_data(file_path):
    with open(file_path, 'r',encoding='utf-8') as csvfile:
        ...
        for i,row in enumerate(reader,start=1):
            ...
            cat = Category.objects.filter(name=row['category'])
            if not len(cat):
                cat = Category()
            ...
            product = Products.objects.create(
                ...
                price=row['price'],
                rating=row['rating'],
                review_count=random.randint(3, 100),
                stock=row['stock'],
                sold=random.randint(2, 100),
                category=cat,
                discount=row['discountPercentage'],
                sku=row['sku'],
                weight=row['weight'],
                created_at=row['meta_createdAt'],
                updated_at=row['meta_updatedAt'],
            )
            product.save()

            # Download and save the associated image
            # images = row['images']
            # for image_url in images.split(','):
            #     download_and_save_image(product, image_url)


def Uploading(request):
    dataset_path = 'static/product_data.csv'
    import_data(dataset_path)
    return Response({"Done Posting"})